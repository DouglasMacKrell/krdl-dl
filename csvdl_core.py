...
def login_to_krdl(username: str, password: str) -> str | None:
    """
...
def scrape_krdl_page(url: str, auth_cookies: str | None = None) -> list[str]:
    """
...
def extract_urls_from_text(file_path: str) -> list[str]:
    ...
    urls: list[str] = []
    try:
        with open(expand(file_path), encoding="utf-8", errors="ignore") as f:
            text = f.read()
...
def prepare_jobs(
    urls: list[str], ext: str, target_dir: Path, auth_cookies: str | None = None
) -> list[Job]:
    """
    ...
    for url in urls:
        ...
        job = Job(url=url, ext=ext, status="QUEUED")
        jobs.append(job)
